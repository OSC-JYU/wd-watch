...
describe('Watchlist check', () => {
		...
		it('should check changes in wathclist', (done) => {
			http.execute(url)
				...
				.end((err, res) => {
					res.should.have.status(200);
					res.body.should.be.a('object');
					chai.expect(res.body).to.have.property('report');
					done();
				});
		});
	});
	...
	describe('/GET checked item', () => {
		it('item should still exist in watchlist', (done) => {
			http.execute(url)
				.get('/api/watchlist/' + venus_id)
				.end((err, res) => {
					res.should.have.status(200);
					res.body.should.be.a('object');
						chai.expect(res.body).to.have.property('_id', venus_id);
					done();
				});
		});
	});
